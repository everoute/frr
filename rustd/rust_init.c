...
struct zebra_privs_t rustd_privs = {
#if defined(FRR_USER)
	.user = FRR_USER,
#endif
...
static const struct frr_yang_module_info *const rustd_yang_modules[] = {};

/* clang-format off */
FRR_DAEMON_INFO(rustd, RUST,
		.vty_port = RUSTD_VTY_PORT,
		.proghelp = "Implementation of the RUST daemon template.",

		.privs = &rustd_privs,

		.yang_modules = rustd_yang_modules,
		.n_yang_modules = array_size(rustd_yang_modules),
